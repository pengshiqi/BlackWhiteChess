...
int Board::evaluate()
{
  // int weight[6] = {100, 400, 200, 80, 200, 20};
  int weight[5] = {100, 400, 200, 80, 20};
  int p, c, l, m, f, d;
  ...
  return weight[0] * p + weight[1] * c + weight[2] * l + weight[3] * m + weight[4] * d;
}
...
int find_the_best(int *arr)
{
  ...
  for (int i = 0; i < 8; i++)
    for (int j = 0; j < 8; j++)
    {
      if (B.map[i][j] == 0 and B.is_legal_position(i, j, 1))
      {
          Board new_B = B;
          new_B.click(i, j, 1);
